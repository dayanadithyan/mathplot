...
def evaluate_vector_expression(expression, variables=None, dimensions=3):
    ...
    if not isinstance(result, (np.ndarray, list, tuple, Vector)):
        raise ValueError(
            f"Expression must return a vector, got {type(result)}")
    ...
    if result.size != dimensions:
        raise ValueError(
            f"Expression must return a {dimensions}D vector, got {result.size}D")
...
def evaluate_matrix_expression(expression, variables=None, shape=(3, 3)):
    ...
    if not isinstance(result, (np.ndarray, list, tuple, Matrix)):
        raise ValueError(
            f"Expression must return a matrix, got {type(result)}")
    ...
    if result.shape != shape:
        raise ValueError(
            f"Expression must return a {shape} matrix, got {result.shape}")
    ...
    for i in range(2, int(math.sqrt(limit)) + 1):
        ...
        if progress_callback and i % 100 == 0:
            ...
            if not progress_callback(progress,
                                     f"Sieving {i}/{int(math.sqrt(limit))}"):
                return []  # Cancelled
    ...
    if sequence_type == 'FIBONACCI':
        ...
        for i in range(length):
            if progress_callback:
                ...
                if not progress_callback(
                        progress, f"Generating term {i+1}/{length}"):
        ...
        for i in range(1, length + 1):
            if progress_callback:
                progress = i / length
                if not progress_callback(
                        progress, f"Generating term {i}/{length}"):
                    return sequence  # Cancelled
...
def parse_matrix(matrix_str):
    ...
    try:
        ...
        if len(set(row_lengths)) != 1:
            raise ValueError(
                f"Matrix rows have inconsistent lengths: {row_lengths}")
