...
def validate_function_args(func: Callable[..., R]) -> Callable[..., R]:
    ...
    def wrapper(*args, **kwargs):
        ...
        for param_name, param_value in bound_args.arguments.items():
            ...
            if not isinstance(param_value,
                              annotation) and annotation is not Any:
                raise TypeError(
                    f"Parameter '{param_name}' must be of type {annotation.__name__}, got {type(param_value).__name__}")
...
def validate_and_report(
        self, validations: List[Tuple[bool, str, str]]) -> bool:
...
def wrap_exception_with_context(
        func_name: str) -> Callable[[Exception], Exception]:
