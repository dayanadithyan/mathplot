...
def create_domain_coloring_material(name):
    """Create a material for domain coloring of complex functions.
    ...
    # For HSV coloring based on argument
    hsv = nodes.new('ShaderNodeHSV')
    
    # For brightness based on magnitude
    ...
    hsv.location = (0, 100)
    math_length.location = (0, -100)
    
    # Connections - Complex arg determines hue, magnitude determines value
    links.new(tex_coord.outputs['Generated'], separate_xyz.inputs['Vector'])
    links.new(separate_xyz.outputs['X'], math_length.inputs[0])
    links.new(separate_xyz.outputs['Y'], math_length.inputs[1])
    
    # Create a constant color that will be modified by HSV
    rgb = nodes.new('ShaderNodeRGB')
    rgb.outputs[0].default_value = (1.0, 0.0, 0.0, 1.0)
    rgb.location = (-200, 100)
    
    # Link RGB to HSV
    links.new(rgb.outputs[0], hsv.inputs[0])
    
    # Use X and Y to calculate angle (atan2)
    # We need to use math nodes for this
    math_atan2 = nodes.new('ShaderNodeMath')
    math_atan2.operation = 'ATAN2'
    math_atan2.location = (-200, 200)
    
    # Link to atan2
    links.new(separate_xyz.outputs['Y'], math_atan2.inputs[0])
    links.new(separate_xyz.outputs['X'], math_atan2.inputs[1])
    
    # Convert atan2 to 0-1 range for hue
    math_divide = nodes.new('ShaderNodeMath')
    math_divide.operation = 'DIVIDE'
    math_divide.inputs[1].default_value = 6.28318  # 2*pi
    math_divide.location = (-50, 200)
    
    # Add 0.5 to normalize negative angles
    ...
    # Connect math nodes
    links.new(math_atan2.outputs[0], math_divide.inputs[0])
    links.new(math_divide.outputs[0], math_add.inputs[0])
    links.new(math_add.outputs[0], hsv.inputs[0])  # Hue
    
    # Set saturation to 1.0
    hsv.inputs[1].default_value = 1.0
    
    # Use magnitude for value but with mapping
    math_magnitude = nodes.new('ShaderNodeMath')
    math_magnitude.operation = 'MULTIPLY'
    math_magnitude.inputs[1].default_value = 0.5
    math_magnitude.location = (200, -100)
    
    # Add minimum brightness
    math_brightness = nodes.new('ShaderNodeMath')
    math_brightness.operation = 'ADD'
    math_brightness.inputs[1].default_value = 0.5
    math_brightness.location = (350, -100)
    
    # Connect magnitude calculation to HSV value
    links.new(math_length.outputs['Value'], math_magnitude.inputs[0])
    links.new(math_magnitude.outputs[0], math_brightness.inputs[0])
    links.new(math_brightness.outputs[0], hsv.inputs[2])  # Value
    
    # Connect HSV to Principled BSDF
    links.new(hsv.outputs[0], principled.inputs['Base Color'])
...
def animate_material_color(material, color_keyframes, frame_start=1, frame_end=100):
    """Animate the color of a material over time.
    
    Args:
        material (bpy.types.Material): The material to animate
        color_keyframes (list): List of (frame, color) tuples, where color is (r,g,b,a)
    ...
    for frame, color in color_keyframes:
        ...
        # Set keyframe
        principled.inputs['Base Color'].default_value = color
        principled.inputs['Base Color'].keyframe_insert('default_value', frame=frame)
        
        # If alpha is changing, animate that too
        if color[3] < 1.0:
            material.blend_method = 'BLEND'
            principled.inputs['Alpha'].default_value = color[3]
            principled.inputs['Alpha'].keyframe_insert('default_value', frame=frame)
...
def register_common_materials():
    ...
    # Create colormap materials
    material_registry['VIRIDIS_COLORMAP'] = create_colormap("Viridis", 10, 'VIRIDIS')
    material_registry['MAGMA_COLORMAP'] = create_colormap("Magma", 10, 'MAGMA')
    material_registry['RAINBOW_COLORMAP'] = create_colormap("Rainbow", 10, 'RAINBOW')
    
    # Create special materials
    material_registry['DOMAIN_COLORING'] = create_domain_coloring_material("DomainColoring_Material")
    material_registry['WIREFRAME'] = create_wireframe_material("Wireframe_Material", (0.8, 0.8, 0.8, 1.0))
